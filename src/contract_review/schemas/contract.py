...
class ContractParty(BaseModel):
    ...
    name: str = Field(
        title="主体名称", description="合同主体名称，例如甲方公司名称或乙方公司名称。"
    )
    role: str | None = Field(
        default=None,
        title="主体角色",
...
class RiskFinding(BaseModel):
    ...
    category: str = Field(
        title="风险类别", description="例如主体资格、付款条款、违约责任、争议解决。"
    )
...
class RevisionSuggestion(BaseModel):
    ...
    revised_clause: str | None = Field(
        default=None, title="建议条款", description="建议替换后的条款文本。"
    )
