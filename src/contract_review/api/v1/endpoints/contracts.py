...
async def review_contract_version(
    ...
    # Local/desktop mode executes the async graph synchronously. Keep that
    # compatibility path outside FastAPI's already-running event loop.
    task = await asyncio.to_thread(task_service.enqueue_or_run, task.task_id)
