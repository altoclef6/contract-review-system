...
class ReportService:
    ...
    def save_docx_report(self, review_id: str, report: dict[str, Any]) -> Path:
        ...
        for item in report.get("依据检索", []):
            doc.add_paragraph(f"{item.get('来源', '')}：{item.get('内容', '')}", style="List Bullet")
        doc.save(path)
        return path
    ...
    def save_pdf_report(self, review_id: str, report: dict[str, Any]) -> Path:
        ...
        normal = ParagraphStyle("ChineseNormal", parent=styles["Normal"], fontName="ChineseFont", fontSize=9, leading=15)
        title = ParagraphStyle("ChineseTitle", parent=styles["Title"], fontName="ChineseFont", fontSize=18, leading=24)
        heading = ParagraphStyle("ChineseHeading", parent=styles["Heading2"], fontName="ChineseFont", fontSize=13, leading=18)
        ...
        story.extend([Spacer(1, 10), Paragraph("审查摘要", heading), Paragraph(str(report.get("审查摘要", "")), normal)])
        story.extend([Spacer(1, 10), Paragraph("风险点", heading)])
        for item in report.get("风险点", [])[:20]:
            story.append(Paragraph(f"{item.get('风险编号', '')} [{item.get('风险等级', '')}] {item.get('风险标题', '')}", normal))
        ...
        for item in report.get("修改建议", [])[:20]:
            ...
            story.append(Spacer(1, 5))
        document = SimpleDocTemplate(str(path), pagesize=A4, rightMargin=16 * mm, leftMargin=16 * mm, topMargin=16 * mm, bottomMargin=16 * mm)
