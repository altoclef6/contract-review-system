...
规则中的“缺失条款”会将未出现指标的合同标为待人工复核，适合作为召回优先的初筛，尚未用 Gold Standard 校准误报。数据库模型已迁移，但风险持久化写入服务尚未完成。现有知识 Markdown 不能代表完整、权威或持续更新的法律数据库。
...
安全回滚分支为 `backup/enterprise-upgrade-20260713-035954`。不要使用破坏性 reset；需要回退时先保存当前改动，再从该分支创建新的恢复分支。数据库先备份，再执行针对 `20260713_0003` 的 downgrade。

## 27. 下一阶段建议

优先级顺序：Token 撤销与登录限流 → 全资源授权矩阵 → 上传 ZIP/OCR 安全 → 风险持久化和人工审核 API → 结构化知识检索 → Celery 幂等状态机 → Gold 数据与评测 → 前端闭环 → CI/覆盖率。
...
14. **Token 还缺什么？** 需要持久 token_version 或 refresh token family，密码修改与撤销后立即失效。
...
18. **前端当前风险是什么？** 构建成功但 bundle 偏大，完整人工复核与版本闭环页面仍未落地。
19. **Docker 验证到什么程度？** Compose 语法通过，镜像构建和完整服务启动未真实测量。
20. **下一步最重要是什么？** 完成 Token 生命周期和全资源授权矩阵，再补持久化闭环与真实评测。
