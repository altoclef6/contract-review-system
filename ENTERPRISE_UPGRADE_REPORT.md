...
- 上传审查默认使用模型或可解释内容规则自动分类，保留置信度、证据和人工覆盖记录，并在成功后自动创建合同档案与初始版本。
- 修复合同风险统计包含文本字段时导致合同中心列表 500 的回归；报告中心和风险台账新增基于真实授权数据的指标、饼图和明细筛选。
...
## 7-12. 验证结果（2026-07-19 发布候选复核）

- 当前测试总数：102。
- 通过：102。
- 失败：0。
- 真实覆盖率：83.85%，发布门槛为 80%。
- Ruff 通过；Mypy 严格检查 124 个源文件通过；Python 与 Node 依赖审计未发现已知漏洞。
- 前端构建：成功，2409 modules；组件按需加载后最大图表 chunk 为 554.08 kB（gzip 189.47 kB）。
- Alembic 单一 head；全新库升级/回退/再升级与带既有数据的往返均通过。
- Docker：`docker compose config --quiet`、后端/前端镜像构建和六服务健康检查本地通过；真实容器链路完成登录、虚构合同上传、自动分类、自动归档以及报告/风险统计验证。
